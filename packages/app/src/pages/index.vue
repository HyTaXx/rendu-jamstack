...
<template>
  <div class="container">
    <div class="flex flex-col items-center gap-y-4 py-[180px]">
      ...
      <input v-model="search.search" placeholder="Chercher une recette" type="text" class="pl-10 z-0 w-full pr-4 py-2 border rounded-lg focus:outline-none focus:border-pink-600" />
    </div>
    <div class="flex flex-row gap-6 py-6">
      <button v-for="tag in search.results[0]?.tags" @click="search.selectedTag = tag.name">{{ tag.name }}</button>
    </div>
      <div class="flex flex-wrap justify-around">
        <div v-for="result in search.filteredResults" class="flex flex-col p-8">
          <div class="absolute flex gap-6 ml-2">
            <p v-for="tag in result.tags" class="bg-pink-600 py-1 px-2 rounded-xl text-white">{{ tag.name }}</p>
          </div>
          <NuxtImg :src="result.images[0].url" :alt="result.title" class="w-[600px]"/>
          <h3>{{ result.title }}</h3>
          <div class="flex flex-row justify-around">
            <div class="flex flex-col justify-center w-[200px]">
              ...
              <p class="text-center">Duration : <span class="font-bold">{{ result.duration }}</span> min</p>
            </div>
            <div class="flex flex-col justify-center w-[200px]">
              ...
              <p class="text-center">Difficulty : <span class="font-bold">{{ result.difficulty }}/5</span></p>
            </div>
            <div class="flex flex-col justify-center w-[200px]">
              ...
              <p class="text-center">Cost : <span class="font-bold">{{ result.price }}</span> €</p>
            </div>
          </div>
          <NuxtLink :to="`/recettes/${result.slug}`" class="bg-pink-600 py-4 px-8 text-white rounded-xl m-auto">Voir la recette</NuxtLink>
        </div>
      </div>
    </div>
  </div>
